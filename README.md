...
コマンド、通知、プレビュー、設定画面は、Obsidian の表示言語が日本語なら日本語で表示します。その他の表示言語では英語を使用します。

`month-start` や `weekday=mon` など、テンプレートへ記述する条件名と設定値は表示言語にかかわらず共通です。

## 設定

`Task Gate コメントを残す`（英語表示では `Keep Task Gate comments`）を有効にすると、生成後や挿入後も `<!-- dtg: ... -->` を残します。初期値は無効です。

設定画面には、利用可能な条件、値、HTML コメントの記述例を掲載しています。
